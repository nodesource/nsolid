...
GrpcMetadata GrpcAgent::BuildRpcMetadata(const std::string& agent_id,
  ...
  GrpcMetadata metadata = {
    ...
    {"nsolid-version",
     "node-v" NODE_VERSION_STRING "-nsolid-v" NSOLID_VERSION_STRING},
  };
  if (!saas.empty()) {
...
GrpcAgent::GrpcAgent(): hooks_init_(false),
  ...
  auto cert_path = per_process::system_environment->Get(kNSOLID_GRPC_CERTS);
  if (cert_path.has_value()) {
    Debug("Using certs from: %s\n", cert_path.value().c_str());
    int r = ReadFileSync(&custom_certs_, cert_path.value().c_str());
    if (r != 0) {
...
/*virtual*/
void GrpcAgent::on_command_received(grpcagent::CommandRequest&& request) {
...
void GrpcAgent::on_asset_stream_done(const ::grpc::Status&, AssetStor&& stor) {
