...
const start = require('./mocks/apiserver')

test('client creation', (t) => {
  start(6750, (stop) => {
    const client = new Client('http://localhost:6750/api/', (err, c) => {
...
test('client empty body', (t) => {
  start(6750, (stop) => {
    ...
    client.run('empty', {timeout: 20}, (err, res, reply) => {
      t.ok(err, 'has error')
      t.equals(reply, undefined)
      stop(() => t.end())
    })
  })
})
