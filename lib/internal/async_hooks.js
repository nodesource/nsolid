...
function promiseInitHook(promise, parent) {
  ...
  if (nsolidPromiseTrackingEnabled) {
    // Increment promise counter
    nsolidApi.nsolid_counts[
      nsolidApi.nsolid_consts.kAsyncPromiseCreatedCount]++;
  }
}
...
function promiseResolveHook(promise) {
  ...
  if (nsolidPromiseTrackingEnabled) {
    // Increment promise counter
    nsolidApi.nsolid_counts[
      nsolidApi.nsolid_consts.kAsyncPromiseResolvedCount]++;
  }
}
...
function disablePromiseHookIfNecessary() {
  if (!wantPromiseHook) {
    ...
    if (nsolidPromiseTrackingEnabled) {
      const promiseHooks = require('internal/promise_hooks');
      stopPromiseHook = promiseHooks.createHook({
        init: nsolidPromiseInitHook,
        settled: nsolidPromiseResolveHook,
      });
    }
  }
}

// For N|Solid promise tracking
let nsolidPromiseTrackingEnabled = false;

function nsolidPromiseInitHook(promise, parent) {
  nsolidApi.nsolid_counts[
    nsolidApi.nsolid_consts.kAsyncPromiseCreatedCount]++;
}

function nsolidPromiseResolveHook(promise) {
  nsolidApi.nsolid_counts[
    nsolidApi.nsolid_consts.kAsyncPromiseResolvedCount]++;
}

function trackPromises(enable) {
  nsolidPromiseTrackingEnabled = enable;
  if (!wantPromiseHook) {
    if (stopPromiseHook) stopPromiseHook();
    if (enable) {
      const promiseHooks = require('internal/promise_hooks');
...
function loadNsolid() {
  const api = internalBinding('nsolid_api');
  nsolidApi.nsolid_consts = api.nsolid_consts;
  nsolidApi.nsolid_counts = api.nsolid_counts;
  nsolidApi.traceFlags = api.trace_flags;
}
