...
function requestOnConnect(headersList, headersObject, options) {
  const session = this[kSession];
...
class ClientHttp2Session extends Http2Session {
  ...
  request(headersParam, options) {
    ...
    const reqData = this._requestPrepare(headersParam, options);

    if (!clientTracingChannels.hasSubscribers) {
      return this._requestFinalize(headersParam, reqData);
    }

    const context = { headers: reqData.headersObject,
                      headersUpdated: false,
                      options: reqData.options };
    return clientTracingChannels.start.runStores(context, () => {
      try {
        const data = context.headersUpdated ?
          { ...reqData,
            headersObject: context.headers,
            headersList: buildNgHeaderString(context.headers) } : reqData;
        const stream = this._requestFinalize(headersParam, data);
        stream[kTracingContext] = context;
        return stream;
      } catch (e) {
        context.error = e;
  ...
  _requestPrepare(headersParam, options) {
    if (this.destroyed)
      throw new ERR_HTTP2_INVALID_SESSION();
  ...
  _requestFinalize(headersParam, reqData) {
    const {
      authority,
      headersObject,
      headersList,
    ...
    const onConnect = reqAsync.bind(requestOnConnect.bind(stream, headersList, headersObject, options));
    if (this.connecting) {
