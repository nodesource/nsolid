...
const { addPackage } = require('internal/nsolid_module');
...
function read(jsonPath, { base, specifier, isESM } = kEmptyObject) {
  ...
  const result = {
    ...
    exports: undefined,
    imports: undefined,
    // These 5 new properties are needed for N|Solid package tracking
    version: undefined,
  ...
  if (isResultValid) {
    ...
    if (ObjectPrototypeHasOwnProperty(parsed, 'version')) {
      result.version = parsed.version;
    }

    if (ObjectPrototypeHasOwnProperty(parsed, 'dependencies')) {
      result.dependencies = parsed.dependencies;
    }

    if (ObjectPrototypeHasOwnProperty(parsed, 'optionalDependencies')) {
      result.optionalDependencies = parsed.optionalDependencies;
    }

    if (ObjectPrototypeHasOwnProperty(parsed, 'bundledDependencies')) {
      result.bundledDependencies = parsed.bundledDependencies;
    }

    if (ObjectPrototypeHasOwnProperty(parsed, 'nsolid')) {
      result.nsolid = parsed.nsolid;
    }
