...
wrapModule('pino', 'lib/proto.js', '>=7.x', (mod) => {
  ...
  const syms = ObjectGetOwnPropertySymbols(proto);
  // Using .find() would be nice, but since we are in core and want to avoid array prototype pollution or 
  // missing methods depending on node version, filter is safer or a simple loop.
  const writeSym = syms.filter((s) => s.toString() === 'Symbol(pino.write)')[0];
  
  if (writeSym) {
    wrapMethod(proto, writeSym, (write) => {
