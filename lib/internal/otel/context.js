...
class AbstractAsyncHooksContextManager {
   ...
   * @returns {Function|EventEmitter} The target that was passed in
   */
  bind(context, target) {
   ...
   * @returns {EventEmitter} The EventEmitter that was passed in
   */
...
class AsyncLocalStorageContextManager extends AbstractAsyncHooksContextManager {
  constructor() {
    super();
    this._asyncLocalStorage = null;
  }

  active() {
    const context = this._getALS().getStore();
    if (!context) {
  ...
  with(context, fn, thisArg, ...args) {
    ...
    return this._getALS().run(context, cb, ...args);
  }
  ...
  _getALS() {
    return this._asyncLocalStorage ||= new async_hooks.AsyncLocalStorage();
  }
}
