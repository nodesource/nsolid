...
function prepareMainThreadExecution(expandArgv1 = false, initializeModules = true) {
  ...
  // nsolid_api ArrayBuffers are configured at this point when the EnvInst
  // instance has already been created.
  binding._setupArrayBuffers();
...
function prepareExecution(options) {
  ...
  // As internal/async_hooks is now also part of the snapshot, load the nsolid
  // data at this point.
  require('internal/async_hooks').loadNsolid();
  ...
  if (isMainThread) {
    ...
    // Running the nsolid loader will queue nsolid to run start() based on whether
    // NSOLID_DELAY_INIT was given or not.
    require('internal/nsolid_loader').init();
  ...
  require('internal/nsolid_promise_tracking').init();
