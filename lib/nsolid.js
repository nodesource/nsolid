...
function updateConfig(config = {}) {
  ...
  for (const key in config) {
    ...
    // TODO(santi): Implement validation for every property
      ...
      const value = normalizePositiveFinite(config[key], undefined);
      if (value !== undefined) {
        nsolidConfig[key] = value;
      }
    } else {
...
function initializeConfig(nsolidConfig) {
  ...
  nsolidConfig.contCpuProfile =
    envToBool(process.env.NSOLID_CONT_CPU_PROFILE) ||
    pkgConfig.nsolid.contCpuProfile ||
    false;

  nsolidConfig.contCpuProfileInterval =
    +process.env.NSOLID_CONT_CPU_PROFILE_INTERVAL ||
    pkgConfig.nsolid.contCpuProfileInterval ||
    30000;  // Default: 30 seconds

  // OTLP Agent configuration
  if (nsolidConfig.otlp) {
    const cfg = process.env.NSOLID_OTLP_CONFIG || pkgConfig.nsolid.otlpConfig;
    nsolidConfig.otlpConfig =
      parseOTLPConfig(cfg, nsolidConfig.otlp);
    if (!nsolidConfig.otlpConfig &&
      !(nsolidConfig.otlp === 'otlp' && !cfg)) {
      nsolidConfig.otlp = null;
    }
  }
...
// Parse a raw value (env string, pkg value, or JS API input) to a positive
// finite number.  Returns the numeric value when valid, otherwise `fallback`.
function normalizePositiveFinite(raw, fallback) {
  if (raw == null)
    return fallback;
...
/**
 * @function on
 * @description registers a custom command handler. Check out
 *   {@link https://docs.nodesource.com/nsolid/4.0/docs#custom-commands|Custom Commands}
...
function readPackageJson(path) {
  ...
  try {
    // TODO(trevnorris): This is a stop gap until the internal JSON reader
    // is fixed to support all the fields that we need.
    const binding = internalBinding('fs');
    const { fs: { O_RDONLY } } = internalBinding('constants');
    const data = binding.readFileUtf8(path, O_RDONLY);
...
function parseOtlpConfig(config) {
  ...
  if (config.protocol && !['http', 'grpc'].includes(config.protocol)) {
    return null;
  }
