...
const { ARGV } = require('internal/nsolid_loader');
...
const { basename, dirname, relative, resolve, sep, toNamespacedPath } =
  require('path');
...
const notEnumerable = [ 'parseToken',
                        'tokenKeys',
                        'validateToken' ];
const notEnumerableDescriptor = { __proto__: null, enumerable: false };
for (let i = 0; i < notEnumerable.length; ++i)
  ObjectDefineProperty(start, notEnumerable[i], notEnumerableDescriptor);
...
function initializeConfig(nsolidConfig) {
  // Application name
  const baseTitle = basename(process.title);
  ...
  nsolidConfig.app =
    ...
    pkgConfig.name ||
    ((baseTitle !== 'node') && (baseTitle !== 'nsolid') && process.title) ||
    DEFAULT_APPNAME;
...
function scanForModules(mod_dir) {
  let contents;
  ...
  // Check for package.json in each folder.
