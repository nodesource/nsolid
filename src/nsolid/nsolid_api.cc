...
void EnvList::WriteLogLine(SharedEnvInst envinst, LogWriteInfo info) {
  ...
  if (envlist->exiting_.load(std::memory_order_relaxed)) {
    return;
  }

  size_t s = envlist->on_log_write_q_.enqueue({ envinst, std::move(info) });
  // Might call send() when logs are being processed, but false positives
  // are okay.
  if (s == 1) {
    int er = envlist->log_written_msg_.send();
    CHECK_EQ(er, 0);
  }
}
...
void EnvList::gen_ptiles_cb_(ns_timer*) {
  ...
  for (auto entry : env_map) {
    ...
    auto harvest = [](std::unique_ptr<AttributesHashMap>& hashmap,
                      std::shared_ptr<const std::vector<PointDataAttributes>>& points_ptr) {
      auto new_points = std::make_shared<std::vector<PointDataAttributes>>();
      hashmap->GetAllEnteries(
          [&new_points](const MetricAttributes& attrs,
                    opentelemetry::sdk::metrics::Aggregation& agg) -> bool {
      ...
      hashmap = std::make_unique<AttributesHashMap>(100);
    };
...
void EnvInst::add_metric_datapoint_(MetricsStream::Type type,
  ...
  auto aggregate_into_hashmap = [&value](
    ...
    auto factory = []() -> std::unique_ptr<opentelemetry::sdk::metrics::Aggregation> {
...
void BindingData::SlowPushServerBucket(
  ...
  if (args[5]->IsString()) {
    ...
    route = *String::Utf8Value(isolate, route_s);
  }
