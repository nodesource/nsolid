...
void NSolidHeapSnapshot::take_snapshot(SharedEnvInst envinst,
  ...
  auto it = snapshotter->threads_running_snapshots_.find(envinst->thread_id());
  ...
  if (snapshot == nullptr) {
    ...
    DataOutputStream<HeapSnapshotStor, v8::HeapSnapshot> stream(
      [](std::string snapshot_str, HeapSnapshotStor* stor) {
