...
function registerHook(state) {
  binding.registerJSCodeEventCallback(state.hookIndex, common.mustCallAtLeast((fnName, tid) => {
    state.called = true;
...
if (isMainThread) {
  binding.registerSelfDisposingCodeEventHook();
  binding.triggerCodeEvent();
  setTimeout(common.mustCall(() => {
    assert.ok(binding.selfDisposingCodeEventHookCompleted());
  }), 500);
}
