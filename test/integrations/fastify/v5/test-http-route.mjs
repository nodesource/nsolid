...
function createMetricsTimeout(ms) {
  return new Promise((_, reject) => {
    setTimeout(() => {
      reject(new Error(`Timed out waiting for metrics after ${ms}ms`));
    }, ms);
  });
}
...
async function runTest() {
  ...
  const waitForMetrics = Promise.race([
    metricsPromise,
    createMetricsTimeout(metricsTimeoutMs),
  ]);
  ...
  try {
    ...
    await fastify.close().catch((err) => {
      console.error('Failed to close Fastify server', err);
    });
    try {
      grpcServer.close();
    } catch (err) {
      console.error('Failed to close gRPC server', err);
    }
  }
}
